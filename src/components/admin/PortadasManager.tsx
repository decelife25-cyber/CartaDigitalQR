...
import { CalendarClock, CheckCircle2, Pencil, Plus, Trash2, X } from 'lucide-react';
...
export default function PortadasManager({ configuracionId, onPortadaActualChange }: Props) {
  ...
  const load = async () => {
    ...
    try {
      const { data, error: queryError } = await supabase.from('portadas_carta').select('*').eq('configuracion_id', configuracionId).order('created_at', { ascending: true });
  ...
  const scheduled = useMemo(() => portadas.filter((item) => isScheduledNow(item, now)).sort((a, b) => new Date(b.programada_desde ?? b.created_at).getTime() - new Date(a.programada_desde ?? a.created_at).getTime()), [portadas, now]);
  ...
  const upload = async (file: File) => {
    if (portadas.length >= MAX_PORTADAS) { setError('Has alcanzado el límite de 10 portadas. Elimina una para guardar otra.'); return; }
    ...
    try {
      ...
      if (insertError) { await supabase.storage.from('productos').remove([path]); throw insertError; }
      if (makeActive) {
        const { error: configError } = await supabase.from('configuracion_restaurante').update({ portada_url: publicData.publicUrl, updated_at: new Date().toISOString() }).eq('id', configuracionId);
        if (configError) throw configError;
  ...
  const remove = async (item: Portada) => {
    if (effective?.id === item.id) { setError('No puedes eliminar la portada que está activa ahora mismo. Activa otra antes de eliminarla.'); return; }
  ...
  const startEdit = (item: Portada) => { setEditingId(item.id); setName(item.nombre); setFrom(toLocalInput(item.programada_desde)); setUntil(toLocalInput(item.programada_hasta)); setError(null); };
  ...
  return <section className="mt-2 w-full min-w-0 rounded-xl border bg-[var(--app-surface)] p-2.5" style={{ borderColor: 'var(--app-border)' }}>
    <div className="flex items-start justify-between gap-3"><div><h2 className="text-sm font-extrabold">Portadas de la carta</h2><p className="mt-0.5 text-[9px] text-[var(--app-muted)]">Guarda hasta 10 portadas y cambia entre ellas sin volver a subirlas.</p></div><span className="shrink-0 rounded-full bg-orange-400/10 px-2 py-0.5 text-[9px] font-bold text-orange-500">{portadas.length}/{MAX_PORTADAS}</span></div>
    ...
    {loading ? <div className="flex justify-center py-8 text-xs text-[var(--app-muted)]">Cargando portadas…</div> : <>
      <div className="mt-2 grid gap-2 sm:grid-cols-2">{portadas.map((item) => {
        ...
        return <article key={item.id} className={`overflow-hidden rounded-xl border ${isEffective ? 'border-orange-400/70 ring-1 ring-orange-400/20' : ''}`} style={{ borderColor: isEffective ? undefined : 'var(--app-border)' }}>
          ...
          <div className="p-2">{editingId === item.id ? <div className="space-y-2">
      ...
      <div className="mt-2 flex items-center gap-2"><input ref={inputRef} type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file); }} /><button type="button" onClick={() => inputRef.current?.click()} disabled={busy || portadas.length >= MAX_PORTADAS} className="flex h-9 flex-1 items-center justify-center gap-1.5 rounded-md bg-orange-400 px-3 text-[10px] font-extrabold text-[#111] disabled:opacity-40"><Plus size={14} />Añadir portada</button><span className="text-[9px] text-[var(--app-muted)]">Máx. 10 MB</span></div>
    </>}
  </section>;
}
