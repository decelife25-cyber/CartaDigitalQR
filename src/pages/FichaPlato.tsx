          ...
          <div className="mt-6 pt-5" style={{ borderTop: '1px solid var(--app-border)' }}>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {producto.alergenos.map((alergeno) => (
                <div key={alergeno.id} className="flex min-h-10 min-w-0 items-center gap-1.5 rounded-xl px-2.5 py-1.5" style={{ background: 'var(--app-surface-soft)', border: '1px solid var(--app-border)' }}>
                  <AlergenoIcon icono={alergeno.icono} nombre={alergeno.nombre} />
                  <span className="min-w-0 flex-1 text-[13px] font-semibold leading-tight whitespace-normal break-normal">{alergeno.nombre}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 p-3" style={{ background: 'linear-gradient(to top, var(--app-bg) 72%, transparent)' }}>
        <div className="mx-auto max-w-xl">
          <button
            ...
            className={clsx(
              'flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl px-5 text-[16px] font-extrabold shadow-xl transition-transform active:scale-[.99]',
