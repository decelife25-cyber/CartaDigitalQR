  ...
  return (
    <main className="min-h-[100dvh] w-full" style={{ background: 'var(--app-bg)', color: 'var(--app-text)' }}>
      <div className="mx-auto w-full max-w-2xl">
          ...
          {familias.map((familia) => (
            <button key={familia.id} type="button" onClick={() => navigate(`/familias/${familia.id}`)} className="flex min-h-[72px] w-full items-center border-b text-left transition-colors active:brightness-95" style={{ background: 'var(--app-surface)', borderColor: 'var(--app-border)' }}>
              <div className="relative ml-3 h-[58px] w-[74px] shrink-0 overflow-hidden rounded-lg bg-stone-200">
                {familia.foto_url ? <img src={familia.foto_url} alt={`Foto de ${familia.nombre}`} className="h-full w-full object-cover" loading="lazy" /> : <div className="flex h-full w-full items-center justify-center" style={{ color: 'var(--app-muted)' }}><Utensils size={20} strokeWidth={1.6} /></div>}
        ...
        {familias.length === 0 && <div className="px-4 py-12 text-center" style={{ color: 'var(--app-muted)' }}><Utensils className="mx-auto mb-3 h-8 w-8 opacity-50" strokeWidth={1.5} /><p className="text-sm">No hay familias disponibles en este momento.</p></div>}
      </div>
