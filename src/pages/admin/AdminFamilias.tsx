  ...
  return (
    <div className="w-full min-h-[calc(100dvh-4rem)]" style={{ color: 'var(--app-text)' }}>
      <header className="flex items-center gap-2 border-b pb-3" style={{ borderColor: 'var(--app-border)' }}>
        <Link to="/admin" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl" aria-label="Volver al panel privado">
          <ArrowLeft size={23} />
        </Link>
        <div className="min-w-0">
          <h1 className="text-2xl font-extrabold tracking-tight">Familias</h1>
          <p className="mt-0.5 text-[10px] font-semibold uppercase tracking-wider" style={{ color: 'var(--app-muted)' }}>
      ...
      {error && (
        <div className="mt-3 rounded-xl border p-3 text-sm" style={{ borderColor: 'rgba(239,68,68,.25)', background: 'var(--app-surface)', color: '#dc2626' }}>
          {error}
      ...
      <section className="mt-3 rounded-2xl border p-2.5" style={{ background: 'var(--app-surface)', borderColor: 'var(--app-border)', boxShadow: 'var(--app-shadow)' }}>
        <div className="flex gap-2">
          <input
            ...
            className="h-11 min-w-0 flex-1 rounded-xl border bg-transparent px-3 text-sm font-medium outline-none focus:border-orange-500"
          ...
          <button
            ...
            disabled={saving}
            onClick={() => void create()}
            className="inline-flex h-11 shrink-0 items-center gap-1.5 rounded-xl bg-orange-500 px-4 text-sm font-extrabold text-white disabled:cursor-wait disabled:opacity-50"
            aria-label="Añadir familia"
          >
            <Plus size={18} />
            <span>Añadir</span>
          </button>
        ...
        {ordered.map((familia, index) => {
          ...
          return (
            <article
              ...
              className={`rounded-2xl border px-2.5 py-2 transition-opacity ${draggedId === familia.id ? 'opacity-50' : 'opacity-100'}`}
              ...
              <div className="flex min-h-12 items-center gap-1.5">
                ...
                {editing ? (
                  <input
                    ...
                    className="h-10 min-w-0 flex-1 rounded-xl border bg-transparent px-2 text-base font-bold outline-none focus:border-orange-500"
                    style={{ borderColor: 'rgba(249,115,22,.55)', color: 'var(--app-text)' }}
                    aria-label="Nombre de la familia"
                  />
                ) : (
                  <div className="min-w-0 flex-1 pr-1">
                    <div className="break-words text-[16px] font-extrabold leading-tight">{familia.nombre}</div>
                  </div>
                )}
                ...
                {editing ? (
                  ...
                  <div className="flex shrink-0 items-center gap-0.5">
                    <button
                      ...
                      className={`min-w-[70px] rounded-xl px-2 py-2 text-[11px] font-extrabold transition-opacity disabled:opacity-40 ${familia.activo ? 'bg-emerald-500/10 text-emerald-600' : 'bg-black/5 text-zinc-500 dark:bg-white/5 dark:text-zinc-400'}`}
                      aria-label={familia.activo ? `Ocultar ${familia.nombre}` : `Mostrar ${familia.nombre}`}
                    >
                      {familia.activo ? 'Visible' : 'Oculta'}
                    </button>
                    <button type="button" onClick={() => startEdit(familia)} className="flex h-10 w-10 items-center justify-center rounded-xl" style={{ color: 'var(--app-muted)' }} aria-label={`Editar ${familia.nombre}`}>
                      <Edit size={19} />
                    </button>
                    <button type="button" disabled={saving} onClick={() => void remove(familia)} className="flex h-10 w-10 items-center justify-center rounded-xl text-zinc-400 hover:text-red-500 disabled:opacity-40" aria-label={`Eliminar ${familia.nombre}`}>
                      <Trash2 size={19} />
                    </button>
