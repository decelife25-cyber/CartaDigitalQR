import { ArrowRight, CookingPot, FolderKanban, Settings2 } from 'lucide-react';
import { Link } from 'react-router-dom';

const sections = [
  { title: 'Familias', description: 'Organiza las categorías de la carta.', icon: FolderKanban, accent: '#8b5cf6', path: '/admin/familias' },
  { title: 'Productos', description: 'Crea, edita y ordena los platos.', icon: CookingPot, accent: '#f59e0b', path: '/admin/productos' },
  { title: 'Configuración', description: 'Ajustes generales de la carta.', icon: Settings2, accent: '#64748b', path: '/admin/configuracion' },
] as const;
    ...
    <div className="mx-auto w-full max-w-5xl px-2 py-2 sm:px-4 sm:py-4">
      ...
      <section className="grid grid-cols-1 gap-2 sm:grid-cols-3 sm:gap-3">
        {sections.map(({ title, description, icon: Icon, accent, path }) => <Link key={title} to={path} className="group flex min-h-28 items-center gap-4 rounded-3xl border p-4 transition-transform hover:-translate-y-0.5 sm:min-h-32 sm:p-5" style={{ background: 'var(--app-surface)', borderColor: 'var(--app-border)', boxShadow: 'var(--app-shadow)' }}>
          <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl" style={{ background: `${accent}18`, color: accent }}><Icon size={31} strokeWidth={2.1} /></span>
