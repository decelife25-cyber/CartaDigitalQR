...
export default function AdminProductoForm() {
  ...
  const handleDelete=async()=>{if(!id||!window.confirm(`¿Eliminar «${nombre}»? Esta acción no se puede deshacer.`))return;setSaving(true);try{await adminApi.deleteProducto(id);navigate('/admin/productos');}catch(e){alert(errorMessage(e));setSaving(false);}};
  const handleSubmit=async(e:FormEvent)=>{e.preventDefault();const clean=nombre.trim();const p=Number(precio.replace(',','.'));if(!clean)return alert('El producto necesita un nombre.');if(!familiaId)return alert('Debes seleccionar una familia.');if(!Number.isFinite(p)||p<0)return alert('Introduce un precio válido.');setSaving(true);try{const data:Partial<Producto>={nombre:clean,descripcion:descripcion.trim()||null,precio:p,familia_id:familiaId,foto_url:fotoUrl.trim()||null,activo,agotado,destacado,sugerido};if(isEditing&&id)await adminApi.updateProducto(id,data,[...selectedAlergenos]);else await adminApi.createProducto(data,[...selectedAlergenos]);navigate('/admin/productos');}catch(e){alert(errorMessage(e));}finally{setSaving(false);}};
  ...
  return <div className="min-h-[calc(100dvh-4rem)] w-screen max-w-none overflow-x-clip" style={{background:'var(--app-bg)',color:'var(--app-text)',width:'100vw',marginLeft:'calc(50% - 50vw)'}}>
    <header className="sticky top-0 z-30 flex h-11 items-center gap-1 border-b px-2" style={{borderColor:'var(--app-border)',background:'color-mix(in srgb, var(--app-bg) 94%, transparent)',backdropFilter:'blur(10px)'}}>
      ...
      {isEditing&&<button type="button" onClick={()=>void handleDelete()} disabled={saving} className="inline-flex h-8 shrink-0 items-center gap-1 rounded-lg px-2 text-[11px] font-extrabold text-red-500"><Trash2 size={15}/><span className="hidden min-[390px]:inline">Eliminar</span></button>}
    ...
    <form id="producto-form" onSubmit={handleSubmit} className="w-full space-y-2 px-0 pb-14 pt-2">
      ...
      <section className="w-full rounded-xl border p-2.5 shadow-sm" style={{background:'var(--app-surface)',borderColor:'var(--app-border)',boxShadow:'var(--app-shadow)'}}><div className="mb-1.5 flex items-end justify-between"><div><h2 className="text-[15px] font-extrabold">Alérgenos</h2><p className="text-[9px]" style={{color:'var(--app-muted)'}}>Selecciona los alérgenos que contiene este artículo.</p></div><span className="rounded-full px-2 py-0.5 text-[10px] font-bold text-emerald-500" style={{background:'var(--app-surface-soft)'}}>{selectedAlergenos.size}</span></div><div className="grid grid-cols-2 gap-1">{alergenos.map(a=>{const selected=selectedAlergenos.has(a.id);return <label key={a.id} title={a.nombre} className="flex min-h-8 cursor-pointer items-center gap-1.5 rounded-lg border px-1.5 transition" style={{borderColor:selected?'rgba(16,185,129,.8)':'var(--app-border)',background:selected?'rgba(16,185,129,.10)':'var(--app-surface-soft)'}}><input type="checkbox" checked={selected} onChange={()=>toggleAlergeno(a.id)} className="sr-only"/><span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-sm border" style={selected?{borderColor:'#10b981',background:'#10b981',color:'#fff'}:{borderColor:'var(--app-muted)',color:'transparent'}}><Check size={11} strokeWidth={3}/></span><AlergenoIcon alergeno={a}/><span className="min-w-0 break-words text-[10px] font-semibold leading-[1.05]" style={{color:'var(--app-text)'}}>{a.nombre}</span></label>})}</div></section>
