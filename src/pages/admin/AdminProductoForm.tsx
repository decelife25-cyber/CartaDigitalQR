  ...
  return <div className="min-h-[calc(100dvh-4rem)] w-screen max-w-none overflow-x-clip" style={{background:'var(--app-bg)',color:'var(--app-text)',width:'100vw',marginLeft:'calc(50% - 50vw)'}}>
    ...
    <form id="producto-form" onSubmit={handleSubmit} className="w-full space-y-2 px-0 pb-14 pt-2">
      <section className="w-full rounded-xl border p-2.5 shadow-sm" style={{background:'var(--app-surface)',borderColor:'var(--app-border)',boxShadow:'var(--app-shadow)'}}>
        ...
        <div className="mt-2 grid grid-cols-3 gap-1.5 border-t pt-2" style={{borderColor:'var(--app-border)'}}><CompactStatus label="Disponible" checked={!agotado} onChange={v=>setAgotado(!v)}/><CompactStatus label="Especialidad" checked={destacado} icon={<span className="text-[11px] leading-none" aria-hidden="true">👨‍🍳</span>} onChange={setDestacado}/><CompactStatus label="Sugerencia" checked={sugerido} icon={<Lightbulb size={11}/>} onChange={setSugerido}/></div>
      </section>
