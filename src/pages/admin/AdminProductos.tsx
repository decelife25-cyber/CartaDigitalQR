...
export default function AdminProductos() {
  ...
  const familiaLabel = familiaId === 'todas'
    ? 'Familia: Todas'
    ...
    <div className="min-h-[calc(100dvh-4rem)]" style={{ color: 'var(--app-text)', width: '100vw', maxWidth: '100vw', marginLeft: 'calc(50% - 50vw)', overflowX: 'clip' }}>
      ...
      <div className="px-2 py-2">
        <div className="relative">
          ...
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar artículos..." className="h-10 w-full rounded-xl border bg-transparent pl-10 pr-10 text-[14px] outline-none focus:border-orange-500" style={{ borderColor: 'var(--app-border)', color: 'var(--app-text)' }} aria-label="Buscar artículos" />
          ...
          return (
            <article key={producto.id} data-product-id={producto.id} className={`grid grid-cols-[minmax(0,1fr)_34px] items-stretch overflow-hidden border-y transition-opacity ${dragging ? 'opacity-45' : 'opacity-100'}`} style={{ background: 'var(--app-surface)', borderColor: 'var(--app-border)', boxShadow: 'var(--app-shadow)' }}>
              <Link to={`/admin/productos/${producto.id}/editar`} className="grid min-w-0 grid-cols-[42px_minmax(0,1fr)_76px] items-center gap-2 py-1.5 pl-2 pr-1" aria-label={`Editar ${producto.nombre}`}>
                {producto.foto_url ? <img src={producto.foto_url} alt="" className="h-10 w-10 rounded-lg object-cover" /> : <div className="flex h-10 w-10 items-center justify-center rounded-lg" style={{ background: 'rgba(148,163,184,.12)', color: 'var(--app-muted)' }} aria-hidden="true"><ImageOff size={17} /></div>}
