...
function portadaProgramadaAhora(portada: PortadaProgramada, now: number) {
  ...
  return (from !== null || until !== null) && (from === null || now >= from) && (until === null || now <= until);
}
...
export default function Portada() {
  ...
  const [pizarraImagenCargada, setPizarraImagenCargada] = useState(false);
  const [horarioAbierto, setHorarioAbierto] = useState(false);
  const textoPizarraRef = useRef<HTMLDivElement | null>(null);

  useEffect(() => {
    let cancelled = false;
    Promise.all([api.getConfiguracion(), api.getSugerencias()]).then(async ([data, productos]) => {
      if (cancelled) return;
      ...
      if (data?.id) {
        const { data: portadas } = await supabase.from('portadas_carta').select('id,image_url,activa,programada_desde,programada_hasta').eq('configuracion_id', data.id);
        if (!cancelled && portadas) {
          const now = Date.now();
          const scheduled = (portadas as PortadaProgramada[]).filter((item) => portadaProgramadaAhora(item, now)).sort((a, b) => new Date(b.programada_desde ?? 0).getTime() - new Date(a.programada_desde ?? 0).getTime());
          setPortadaProgramada(scheduled[0] ?? null);
        }
      }
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      if (!config?.id) return;
      const refresh = async () => {
        const { data } = await supabase.from('portadas_carta').select('id,image_url,activa,programada_desde,programada_hasta').eq('configuracion_id', config.id);
        if (!data) return;
        const now = Date.now();
        const scheduled = (data as PortadaProgramada[]).filter((item) => portadaProgramadaAhora(item, now)).sort((a, b) => new Date(b.programada_desde ?? 0).getTime() - new Date(a.programada_desde ?? 0).getTime());
        setPortadaProgramada(scheduled[0] ?? null);
      };
      void refresh();
    }, 60_000);
  ...
  useLayoutEffect(() => {
    ...
    if (!element || sugerencias.length === 0 || !pizarraImagenCargada) return;
    let frame = 0;
    let cancelled = false;
    const fitText = () => {
  ...
  }, [sugerencias, pizarraAmpliada, pizarraImagenCargada]);
  ...
  const portadaImage = portadaProgramada?.image_url?.trim() || config?.portada_url?.trim() || DEFAULT_PORTADA_IMAGE;
