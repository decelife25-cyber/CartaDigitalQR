    ...
    <main className="relative h-[100dvh] w-full overflow-hidden bg-black">
      ...
      <div className="absolute inset-x-0 bottom-0 z-30 px-4 pb-2">
        <div className="mx-auto w-full max-w-lg">
          ...
          {(direccion || phone) && <div className="mt-2 whitespace-nowrap rounded-lg bg-black/35 px-2 py-1 text-center text-[10px] font-semibold leading-tight text-white shadow-sm backdrop-blur-sm">{direccion && <span>{direccion}</span>}{direccion && phone && <span className="mx-2">|</span>}{phone && <span>{phone}</span>}</div>}
        </div>
      </div>

      {horarioAbierto && horario && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/50 px-5 backdrop-blur-[2px]" role="dialog" aria-modal="true" aria-labelledby="horario-title" onMouseDown={(event) => { if (event.currentTarget === event.target) setHorarioAbierto(false); }}>
          <section className="w-full max-w-sm rounded-3xl border border-white/20 bg-black/85 p-5 text-white shadow-2xl backdrop-blur-md">
            <div className="flex items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-2.5"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white/10"><Clock3 size={21} /></span><h2 id="horario-title" className="text-xl font-extrabold">Horario</h2></div><button type="button" onClick={() => setHorarioAbierto(false)} aria-label="Cerrar horario" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white/10 text-white/90 active:scale-95"><X size={19} /></button></div>
