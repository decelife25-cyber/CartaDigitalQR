...
// Public assets are served by the same Cloudflare deployment.
// The image files themselves will be stored in Supabase Storage.
...
export default function Portada() {
  ...
  return (
    <main className="relative h-[100dvh] w-full overflow-hidden bg-black">
      ...
      {sugerencias.length > 0 && (
        ...
        >
          <div className="relative w-full drop-shadow-[0_8px_18px_rgba(0,0,0,.45)]">
            ...
            <div ref={textoPizarraRef} className="absolute left-[18%] right-[18%] top-[25%] bottom-[8%] flex flex-col gap-[clamp(4px,1.2vw,9px)] overflow-hidden text-white" style={{ fontFamily: '\"Patrick Hand SC\", \"Chalkboard SE\", \"Marker Felt\", \"Segoe Print\", cursive', fontSize: pizarraAmpliada ? '26px' : '16px', lineHeight: 1.12, letterSpacing: '0.025em', textTransform: 'uppercase', textShadow: '0 1px 2px rgba(0,0,0,.9)' }}>
              {sugerencias.map((producto) => <div key={producto.id} className="flex min-w-0 shrink-0 items-start font-normal"><span className="mr-[0.45em] shrink-0">-</span><span className="min-w-0 break-words">{producto.nombre}</span></div>)}
            </div>
          </div>
        </button>
      )}
      ...
      <div className="absolute inset-x-0 bottom-0 z-30 px-4 pb-2"><div className="mx-auto w-full max-w-lg"><div className="grid w-full grid-cols-10 gap-2">
        <button type="button" onClick={reservarMesa} disabled={!urlReservasMesa || reservandoMesa} aria-label={urlReservasMesa ? 'Reservar mesa' : 'Reservar mesa no configurado'} aria-busy={reservandoMesa} className="col-span-3 flex h-12 min-w-0 items-center justify-center gap-1 rounded-xl border border-white/30 bg-black/55 px-1.5 text-[11px] font-bold leading-tight text-white shadow-lg backdrop-blur transition-transform active:scale-[.96] disabled:cursor-not-allowed disabled:opacity-60">{reservandoMesa ? <LoaderCircle size={17} className="shrink-0 animate-spin" /> : <CalendarDays size={17} className="shrink-0" />}<span>{reservandoMesa ? 'Cargando...' : 'Reservar mesa'}</span></button>
      ...
      </div>{(direccion || phone) && <div className="mt-2 whitespace-nowrap rounded-lg bg-black/35 px-2 py-1 text-center text-[10px] font-semibold leading-tight text-white shadow-sm backdrop-blur-sm">{direccion && <span>{direccion}</span>}{direccion && phone && <span className="mx-2">|</span>}{phone && <span>{phone}</span>}</div>}</div></div>
